...
*   **核心目标**：在保持现有 JSON 协议完全不变的前提下，为 WebSocket 连接增加多路复用原始字节流通道。
*   **前置依赖**：仅依赖 Phase 0（Core Transport）。**可独立推进**。

完整设计文档：[`docs/plans/stream-transport.md`](docs/plans/stream-transport.md)

#### 核心架构

利用 WebSocket 原生帧类型（text vs binary）区分控制面与数据面：

```
单一 WebSocket 连接
├── Text 帧 → hiccl JSON 协议（控制面，零改动）
└── Binary 帧 → 流数据通道
    ├── Byte 0: channel_id (uint8, 1-255)
    └── Bytes 1+: 原始载荷
```
...
| 能力 | 说明 |
|:---|:---|
| **Stream 双向通道** | `async send(data: bytes)` / `async recv() -> bytes`，带背压机制 |
| **StreamRegistry** | 每会话管理最多 16 个并发流，自动分配/回收 channel_id |
| **hiccl.js createStream()** | 客户端流 API — `stream.send(data: Uint8Array)` + `stream.onData` |
| **SSE 降级** | Base64 编码的 `hiccl-stream` SSE 事件，对上层 API 透明 |

#### 使用场景

| 场景 | 利用方式 |
|:---|:---|
| **Web 终端**（ttyd/gotty 风格） | xterm.js ↔ Stream ↔ PTY，完整 ANSI 转义序列 + 逐键输入 |
| **在线游戏** | 游戏引擎 ↔ Stream（binary protobuf）↔ 服务端状态机 |
| **实时协作白板** | Canvas 坐标流 ↔ Stream ↔ EventBus 广播 |
| **文件上传/下载** | 分块二进制传输，进度通过 Signal 更新 DOM |
| **IoT 传感器数据** | 高频读数通过 Stream 推送，仪表盘通过 Signal 触发渲染 |

#### 与现有架构的关系

- JSON 协议（action / reconnect / batch）**完全不受影响**
- 现有 `WSTransport.push()`、`RenderScheduler`、`EventBus` 均不变
- 未升级的 hiccl.js 自动忽略二进制帧，功能正常
- `NullTransport` 下 streams 不可用（`supports_streams()` → `False`）

#### 性能指标

- 每消息协议开销：1 字节（channel_id 前缀）
- 终端按键场景：JSON `{"key":"a"}` = 10 字节 → Binary `[1, 0x61]` = 2 字节（**-80%**）
- 1KB 游戏帧 @ 60fps：~60 KB/s（极轻）
...
| **v0.7.0** (Phase 5b) | Datalog-lite API Reference + 查询语法指南 + 示例使用手册 |
| **v0.8.0** (Phase 6) | Stream API Reference + Web 终端构建指南 + 游戏/实时协作场景教程 |
